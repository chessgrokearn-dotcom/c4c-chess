...
export function ProfileEditor({ onClose }: { onClose: () => void }) {
  ...
  const handleSave = () => {
    updatePlayer({ name, description: desc.slice(0, 500), theme, avatar, links: [linkName, linkUrl] as [string, string] });
    onClose();
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }} onClick={onClose}>
      <div style={{ background: '#1f2937', borderRadius: '16px', padding: '24px', maxWidth: '480px', width: '100%' }} onClick={e => e.stopPropagation()}>
        <h3 style={{ marginBottom: '20px', color: 'white' }}>✏️ Профиль</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
            <button onClick={handleSave} style={{ flex: 1, padding: '10px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>💾 Сохранить</button>
            <button onClick={onClose} style={{ flex: 1, padding: '10px', background: '#6b7280', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Закрыть</button>
          </div>
        </div>
      </div>
    </div>
