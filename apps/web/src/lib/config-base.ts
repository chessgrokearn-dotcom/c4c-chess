// apps/web/src/lib/config-base.ts
// 🔹 БАЗОВЫЙ КОНФИГ — НЕ МЕНЯТЬ ПОСЛЕ СОЗДАНИЯ
// Все исправления делаются через патчи в master-config.ts
...
// Базовая фабрика wagmi (может быть исправлена патчами)
export function createWagmiConfigBase(){
  return createConfig({
    chains:[bsc],
    connectors:[
...
// Базовые хелперы подключения (могут быть исправлены патчами)
...
// Базовый CONFIG (будет расширен патчами)
export const CONFIG_BASE={
  ...
  saveProfileToStorage:saveProfile,loadProfileFromStorage:loadProfile,
  createWagmiConfig:createWagmiConfigBase,
  canConnectToMetaMask:canConnectMetaBase,
  canConnectToWalletConnect:canConnectWCBase,
  resetConnectionStates:resetConnBase,
